<template>
  <div class="breadcrumb">
    ...
    <p v-for="(link, idx) in breadcrumbLinks" :key="idx">
...
const breadcrumbLinks = computed<Array<{ link: string; url: string }>>(() => {
  const url = route.path

  const links = url
    .split('/')
    .splice(1)
    .filter((link: string) => !link.includes('0x')) // Filter out address hash
    .map((link: string) => {
      return {
